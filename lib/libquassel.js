                    ...
                    case "Network":
                        ...
                        switch(functionName) {
                            ...
                            case "setConnectionState":
                                ...
                                self.emit('Network.setConnectionState', Network.get(networkId), connectionState);
                                break;
                            case "addIrcChannel":
                                ...
                                self.emit('Network.addIrcChannel', Network.get(networkId), bufferName);
                                break;
                            case "setConnected":
                                var isConnected = obj[4];
                                Network.get(networkId).setConnected(isConnected);
                                self.emit('Network.setConnected', Network.get(networkId), isConnected);
                                break;
                            case "setMyNick":
                                var nick = obj[4];
                                Network.get(networkId).nick = nick;
                                self.emit('Network.setMyNick', Network.get(networkId), nick);
                                break;
                            case "setNetworkName":
                                var networkName = obj[4];
                                Network.get(networkId).networkName = networkName;
                                self.emit('Network.setNetworkName', Network.get(networkId), networkName);
                                break;
                            case "setCurrentServer":
                                var server = obj[4];
                                Network.get(networkId).server = server;
                                self.emit('Network.setCurrentServer', Network.get(networkId), server);
                                break;
                            default:
            ...
            case RequestType.RpcCall:
                switch(className) {
                    case "2displayMsg(Message)":
                        ...
                        self.emit("2displayMsg(Message)", bufferId, simpleMessage);
                        break;
