...
var Quassel = function(server, port, options, loginCallback) {
    ...
    function getMaxBufferId() {
        var maxId = 0, bufferIds;
        self.getNetworksHashMap().forEach(function(network, key){
            bufferIds = network.getBuffers().keys();
            for (var ind in bufferIds) {
                    ...
                    case "Network":
                        ...
                        switch(functionName) {
                            ...
                            case "addIrcChannel":
                                ...
                                self.emit('network.addchannel', networkId, bufferName);
                                break;
                            case "setConnected":
    ...
    function requestBacklog(buffer) {
        var slist = [
            ...
            new qtdatastream.QUserType("MsgId", -1),
            new qtdatastream.QInt(self.options.backloglimit),
            new qtdatastream.QInt(0)
        ];
        self.log('Sending backlog request');
        self.qtsocket.write(slist);
    }
