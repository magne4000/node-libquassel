...
var Quassel = function(server, port, options, loginCallback) {
    ...
    function getMaxBufferId() {
        var maxId = 0, network, buffer, id;
        for (network in self.getNetworks()) {
            for (buffer in self.getNetworks()[network].getBuffers()) {
                id = self.getNetworks()[network].getBuffers()[buffer].id;
                ...
                switch(className) {
                    ...
                    case "BufferViewConfig":
                        switch(functionName) {
                            case "addBuffer":
                                var bufferId = obj[4];
                                var buffer = Network.findBuffer(bufferName);
                                if (buffer !== null){
                                    break;
                                }
                                //TODO hidden buffers
                    ...
                    case "IrcUser":
                        switch(functionName) {
                            case "partChannel":
                                ...
                                var userName = tmp[1];
                                var bufferName = obj[4];
                                Network.get(userNetworkId).getBufferCollection().getBuffer(bufferName).removeUser(userName);
                                self.emit('user.part', Network.get(userNetworkId), bufferName, userName);
                                break;
                            case "quit":
                                var tmp = obj[2].split("/", 2);
                                var userNetworkId = parseInt(tmp[0], 10);
                            ...
                            case "setAway":
                                ...
                                var userName = tmp[1];
                                var isAway = obj[4];
                                Network.get(userNetworkId).getUserByNick(userName).away = isAway;
                                self.emit('user.away', Network.get(userNetworkId), userName, isAway);
                                break;
                            case "setRealName":
                                var tmp = obj[2].split("/", 2);
                                var userNetworkId = parseInt(tmp[0], 10);
                                var userName = tmp[1];
                                var realname = obj[4];
                                Network.get(userNetworkId).getUserByNick(userName).realname = realname;
                                self.emit('user.realname', Network.get(userNetworkId), userName, realname);
                                break;
                            default:
                    ...
                    case "IrcChannel":
                        ...
                        switch(functionName) {
                            ...
                            case "addUserMode":
                                var nick = obj[4];
                                var mode = obj[5];
                                var user = Network.get(bufferNetworkId).getUserByNick(nick);
            ...
            case RequestType.InitData:
                switch(className) {
                    ...
                    case "IrcUser":
                        ...
                        var user = Network.get(networkId).getUserByNick(tmp[1]);
                        user.devour(data);
                    ...
                    case "BufferViewConfig":
                        ...
                        for (ind in data.TemporarilyRemovedBuffers) {
                            buffer = Network.get(data.networkId).getBufferCollection().getBuffer(data.TemporarilyRemovedBuffers[ind]);
                            buffer.setTemporarilyRemoved(true);
                            // TODO emit ?
                        }
                        for (ind in data.RemovedBuffers) {
                            buffer = Network.get(data.networkId).getBufferCollection().getBuffer(data.RemovedBuffers[ind]);
