...
var Quassel = function(server, port, options, loginCallback) {
        ...
        switch (obj.MsgType) {
            ...
            case 'SessionInit':
                ...
                for (i=0; i<obj.SessionState.BufferInfos.length; i++) {
                    ...
                    self.networks.get(ircbuffer.network).buffers.addBuffer(ircbuffer);
    ...
    function getMaxBufferId() {
        ...
        for (network in self.networks.all()) {
            for (buffer in self.networks.all()[network].getBuffers()) {
                id = self.networks.all()[network].getBuffers()[buffer].id;
                        ...
                        switch(functionName) {
                            case "receiveBacklog":
                                ...
                                for (var i=0; i<data.length; i++) {
                                    var buffers = self.networks.get(data[i].bufferInfo.network).buffers;
    ...
    function requestBacklogs(){
        var k, l;
        for (k in self.networks.all()) {
            var buffers = self.networks.all()[k].getBuffers();
            for (l in buffers) {
