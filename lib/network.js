...
Network.remove = function(networkid) {
    delete Network.list[networkid];
};

Network.findBuffer = function(bufferId) {
    var networkid;
    for (networkid in Network.list) {
        if (Network.list[networkid].getBufferCollection().hasBuffer(bufferId)) {
            return Network.list[networkid].getBufferCollection().getBuffer(bufferId);
        }
    }
    return null;
};

Network.removeBuffer = function(bufferId) {
    var buffer = Network.findBuffer(bufferId);
    if (buffer !== null) {
        Network.get(buffer.network).getBufferCollection().removeBuffer(bufferId);
    }
};
